...
import { findUsers, addUser } from '../sql/user'
...
router.get('/fans', async (ctx) => {
    let result = await wechat.getUserList(access_token)
    let users = []
    if (result && result.data && result.data.openid) {
        for (let i = 0; i < result.data.openid.length; i++) {
            users.push(await wechat.fetchUserMess(result.data.openid[i], 'zh_CN'))
        }
    }
    // let databaseUsers = await findUsers()
    ctx.body = users
})
